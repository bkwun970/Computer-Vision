...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel = torch.Tensor(np.tile(kernel, (self.n_channels, 1, 1, 1)))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        width = int(np.floor(kernel.shape[3] / 2))
        height = int(np.floor(kernel.shape[2] / 2))
        
        filtered_image = F.conv2d(x, kernel, padding = (height, width), groups = self.n_channels)
    ...
    def forward(
        ...
        k = self.get_kernel(cutoff_frequency.item())
        low_frequencies = self.low_pass(image1, k)
        high_frequencies = image2 - self.low_pass(image2, k)
