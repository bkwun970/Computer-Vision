...
def create_Gaussian_kernel_1D(ksize: int, sigma: int) -> np.ndarray:
    ...
    array = np.arange(ksize).reshape(ksize, 1)
    mean = np.floor(ksize / 2)

    gaussian = (1/((2 * np.pi) ** (1/2) * sigma)) * (np.exp((-1) * (((array - mean) ** 2) / (2 * (sigma ** 2)))))
...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    """
    sz = (int)(cutoff_frequency * 4 + 1)
    sd = cutoff_frequency

    c = create_Gaussian_kernel_1D(sz, sd)
    r = c.T
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    fshape0 = filter.shape[0]
    fshape1 = filter.shape[1]

    ishape0 = image.shape[0]
    ishape1 = image.shape[1]
    ishape2 = image.shape[2]

    x = int(np.floor(fshape1 / 2))
    y = int(np.floor(fshape0 / 2))
    
    filtered_image = np.empty((ishape0, ishape1, ishape2))
    
    padded = np.zeros((ishape0 + 2 * y, ishape1 + 2 * x, ishape2))

    for i in range(ishape0):
      for j in range(ishape1):
        padded[y + i, x + j] = image[i, j]

    for z in range(ishape2):
      for i in range(ishape0):
        for j in range(ishape1):
          tmp = padded[i:i + fshape0, j:j + fshape1, z]
          mult = np.multiply(tmp, filter)
          sum = np.sum(mult)
          filtered_image[i, j, z] = sum
...
def create_hybrid_image(
    ...
    high_frequencies = np.subtract(image2, my_conv2d_numpy(image2, filter))
